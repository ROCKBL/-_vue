...
<script>
    ...
    import { sortpublish,sortuploadUrl,sortmodify,sortlist } from '@/api/project'
    ...
    methods: {
        ...
        submitForm(formName) {
            ...
            if(this.type=="edite"){
                ...
                sortmodify(this.ruleForm).then(function(response){
                    that.$router.go(-1)
                })
                return
            }

            sortpublish(this.ruleForm).then(function(response){
                that.$router.go(-1)
            })
        },
        // resetForm(formName) {
...
<style>
    ...
    .addProjectSort .picUploader .el-upload {
    ...
    .addProjectSort .picUploader .el-upload:hover {
        border-color: #409EFF;
    }
    .addProjectSort .avatar-uploader-icon {
        font-size: 28px;
        color: #8c939d;
        width: 178px;
        height: 178px;
    ...
    .addProjectSort .avatar {
        width: 178px;
        height: 178px;
        display: block;
    }

    .addProjectSort .servesContentWords{
