<template>
    <div class="ordersTable">
        <el-table border ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" :cell-style="tableStyle" :header-cell-style="tableStyle">
            ...
            <el-table-column prop="orderId" label="订单编号" show-overflow-tooltip width="200"></el-table-column>
            <el-table-column prop="name" label="用户昵称" show-overflow-tooltip width="120"></el-table-column>
            <el-table-column prop="date" label="日期" width="100"></el-table-column>
         
            <el-table-column label="金额" show-overflow-tooltip width="200">
                <template slot-scope="scope">
                    <div>总金额:{{ scope.row.totalMoney }}元（含运费）</div>
                    <div>运费:{{ scope.row.deliveryFee }}元</div>
                    <div>优惠券优惠:{{ scope.row.conponFee }}元</div>
                </template>
            </el-table-column>
            <el-table-column label="实际付款" show-overflow-tooltip width="100">
                <template slot-scope="scope">
                    <div>{{ scope.row.pay }}元</div>
                </template>
            </el-table-column>
