<template>
    <div class="diaryList">
        <div class="sortBtns">
            ...
            <el-input size="small" placeholder="请输入日志名称" v-model="searchInput" class="input-with-select" @keyup.enter.native="searchSort">
        ...
        <div class="sortTable">
            <el-table border ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" :cell-style="tableStyle" :header-cell-style="tableStyle">
                ...
                <el-table-column prop="approve" label="审核状态" show-overflow-tooltip width="100">
                    <template slot-scope="scope">
                        <div v-if="scope.row.approve==0">待审核</div>
                        <div v-if="scope.row.approve==1">审核成功</div>
                        <div v-if="scope.row.approve==2">审核失败</div>
                    </template>
                </el-table-column>

                <el-table-column prop="images" label="日记图片" show-overflow-tooltip width="100"></el-table-column>
